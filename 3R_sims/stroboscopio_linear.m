...
for i=1:length(out.simout(:,1))
    ...
    if mod(i,modulus_)==0
        ...
        line([0,punto_1(1),punto_2(1),punto_3(1)],[0,punto_1(2),punto_2(2),punto_3(2)],color=colors(k,:),LineWidth=2);
        scatter(punto_1(1), punto_1(2),20, colors(k,:),"filled"); % Joint 0
        scatter(punto_2(1), punto_2(2),20, colors(k,:),"filled"); % Joint 0
        scatter(punto_3(1), punto_3(2),20, colors(k,:),"filled"); % Joint 0
        k=k+1;
...
for i=1:length(out.simout1(:,1))
    ...
    if mod(i,modulus_)==0
        ...
        punto_3=double(subs(p3,[q1 q2 q3],[out.simout1(i,1),out.simout1(i,2),out.simout1(i,3)]));
...
colors = viridis(signal_len/modulus_);
k = 1;
for i=1:length(out.simout2(:,1))
    
    if mod(i,modulus_)==0
        
        n=n+1;
        
        punto_1=double(subs(p1,q1,out.simout2(i,1)));
        

        punto_2=double(subs(p2,[q1 q2],[out.simout2(i,1),out.simout2(i,2)]));

        punto_3=double(subs(p3,[q1 q2 q3],[out.simout2(i,1),out.simout2(i,2),out.simout2(i,3)]));
